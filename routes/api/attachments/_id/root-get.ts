...
/**
 * @api {get} /attachments/:id Получить список вложенией
 * @apiVersion 0.0.0
 * @apiName ListAttachments
 * @apiGroup Attachments
 * @apiPermission Юридическое лицо или модератор
 *
 * @apiDescription Сортировка списка вложений производится путем сравнения элементов как массива байт в кодировке UTF-8
 *
 ...
 *
 * @apiSuccess {string[]} attachments Список вложений
...
router.use(new RequestValidator({
  ...
  await res.achain
    .json(async () => {
      ...
      return {
        attachments: (objects.Contents || []).map((v) => v.Key ? v.Key.substring(prefix.length) : undefined),
