...
export default (joi: typeof Joi): Extension => ({
  language: {
    ...
    digits: "must up to 131072 digits before the decimal point; up to 16383 digits after the decimal point",
  ...
  rules: [
    {
      name: "positive",
      validate(params: any, value: string, state: State, options: ValidationOptions): any {
        if (value.startsWith("-")) {
