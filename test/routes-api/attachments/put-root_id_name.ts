...
import {ApiCodes, Bcrypt, Const, EmailNotifications, Env, IEntityInstance, Jwt, S3, Sequelize, Web} from "../../../src";
...
describe("PUT /attachments/:id/:name", () => {
  ...
  it("Bad Request 400 - wrong id (wrong symbols)", async () => {
    await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/attachments/id2/files.zip`)
  ...
  it("Bad Request 400 - wrong id (on less)", async () => {
    await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/attachments/0/files.zip`)
      .set("Authorization", `Bearer ${tokenEntity}`)
      .expect(400, {
        code: PgBigSerialUnitCodes.WRONG_PG_BIGSERIAL,
  ...
  it("Bad Request 400 - wrong id (on more)", async () => {
    await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/attachments/9223372036854775808/files.zip`)
      .set("Authorization", `Bearer ${tokenEntity}`)
      .expect(400, {
        code: PgBigSerialUnitCodes.WRONG_PG_BIGSERIAL,
  ...
  it("Bad Request 400 - wrong name (with slash)", async () => {
    await supertest(Web.instance.app)
      .put(`${Const.API_MOUNT_POINT}/attachments/2/${encodeURIComponent("/files.zip")}`)
      .set("Authorization", `Bearer ${tokenEntity}`)
      .expect(400, {
        code: RegExpUnitCodes.WRONG_REGEXP,
        message: "params.name: value must match regular expression",
  ...
  it("Unauthorized 401 - entity not found", async () => {
    await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/attachments/999/files.zip`)
      .set("Authorization", `Bearer ${await Jwt.signUser({
        id: "999",
