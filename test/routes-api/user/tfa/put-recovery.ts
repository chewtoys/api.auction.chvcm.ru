...
describe("PUT /user/tfa/recovery", () => {
  ...
  it("401 Unauthorized - verify user (jwt must be provided)", async () => {
    await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/user/tfa/recovery`)
      .expect(401, {
  ...
  it("correct token", async () => {
    const res = await supertest(Web.instance.app).put(`${Const.API_MOUNT_POINT}/user/tfa/recovery`)
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
